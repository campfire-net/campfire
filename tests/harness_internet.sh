...
#   3. Launch all architects simultaneously as background processes
#   4. Wait for all to finish OR round timeout (whichever comes first)
#   5. Kill stragglers if timeout hit
#
...
build_architect_prompt() {
    ...
    echo ""
    echo "First, check the coordination campfire for round markers:"
    echo "  Coordination campfire ID: $(cat "$SHARED_DIR/coordination-id.txt")"
    echo ""
    echo "Read it to see any round marker messages and cross-architect discussion."
    echo "Then proceed with your round $round focus."
}
...
build_newcomer_prompt() {
    ...
    echo ""
    echo "You are a brand-new agent. Start with: cf discover"
    echo "Work through the tasks in your mission above. Document everything in your workspace."
}
...
log ""
log "Starting $ROUNDS rounds — 9 architects launch simultaneously per round."
log ""
...
Join this campfire. Post your architect introduction. Then build." \
    ...
    log "  Launching ${#ROUND_AGENTS[@]} agent(s) simultaneously (timeout: ${round_secs}s)..."

    # Launch all agents for this round as background processes
    ...
    for pid in "${ROUND_PIDS[@]}"; do
        ...
        # Wait with timeout: use a background wait + sleep trick
        # bash 4.3+: wait -n is not reliably available; use timeout-aware polling
        if ! wait "$pid" 2>/dev/null; then
            true  # agent exited non-zero — already logged inside run_agent_round
        fi
    done

    # If we hit the round timeout, kill any remaining agent processes
    if $TIMED_OUT; then
