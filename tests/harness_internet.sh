...
build_state_snapshot() {
    ...
    # --- Workspace files (what the agent left for itself) ---
    local workspace_dir="$WORKSPACE/agent-$agent_num"
    if [ -d "$workspace_dir" ]; then
        local file_list
        file_list=$(find "$workspace_dir" -type f -name "*.md" -o -name "*.txt" 2>/dev/null | sort)
        if [ -n "$file_list" ]; then
            echo "### Your workspace files (from previous turns)"
            echo ""
            while IFS= read -r fpath; do
                ...
                echo "#### $fname"
                echo ""
                # Limit to first 50 lines to avoid prompt explosion
                head -50 "$fpath"
                local total
                total=$(wc -l < "$fpath")
                if [ "$total" -gt 50 ]; then
                    echo "... (truncated, $total total lines — read the full file at $fpath)"
                fi
                echo ""
            done <<< "$file_list"
        fi
