...
Read each bead description before the plan is approved. For every bead that involves testable behavior:

1. Identify what an implementer would mock to close the bead fast.
...
4. Add prerequisite beads for any access, credentials, or infrastructure the implementers will need to test for real. These are filed before implementation starts so the human can provision them.

**Output**: Amended bead descriptions with ground-source-truth done conditions. Prerequisite beads for real test infrastructure. Findings posted to the planning campfire tagged `veracity`.

### In swarm-dispatch (implementation phase)

Run in each wave alongside implementers. For every implementation bead in the wave:
...
- **Proven inability**: "Tested approaches A, B, C to hit real Y. A failed because [specific]. B failed because [specific]. C failed because [specific]. Need human to provision [specific thing]." → Becomes a bead for the human. Mock stays but is explicitly marked unproven until the bead is resolved.
...
- Your completion condition is not "reviewed the tests." It's: **every test in this wave either hits ground source truth, or I have an airtight proof that it can't and a bead filed for what the human needs to provide.**

## Process (dispatch phase)

1. Read the wave's bead specs. Note the done conditions and ground-source-truth requirements (if the planning-phase veracity adversary did its job, these should be explicit).
...
7. Post findings to the wave campfire tagged `veracity`.
8. The orchestrator cannot merge the wave until all `veracity` findings are resolved (rewritten to real, or proven-inability bead filed).
9. Close your bead with: `bd close <id> --reason "Veracity audit: N tests verified real, N mocks challenged, N rewritten to real, N proven-inability beads filed."`.
...
Filed: bead <id> — "Provision Polar sandbox API key for real checkout testing" (assigned to human).
```
