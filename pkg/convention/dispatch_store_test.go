...
func TestMemoryDispatchStore_MarkDispatched_InsertIfNotExists(t *testing.T) {
	...
	inserted, err := store.MarkDispatched(ctx, "campfire1", "msg1", "server1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !inserted {
	...
	inserted, err = store.MarkDispatched(ctx, "campfire1", "msg2", "server1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !inserted {
...
func TestMemoryDispatchStore_StatusTransitions(t *testing.T) {
	...
	store.MarkDispatched(ctx, "campfire1", "msg1", "server1")
	store.MarkDispatched(ctx, "campfire1", "msg2", "server1")
...
func TestMemoryDispatchStore_ListStaleDispatches(t *testing.T) {
	...
	store.MarkDispatched(ctx, "campfire1", "old-msg", "server1")
	store.mu.Lock()
	...
	store.MarkDispatched(ctx, "campfire1", "new-msg", "server1")

	// Insert a fulfilled old message — should not appear as stale.
	store.MarkDispatched(ctx, "campfire1", "old-fulfilled", "server1")
	store.mu.Lock()
...
func TestMemoryDispatchStore_CleanupOldDispatches(t *testing.T) {
	...
	store.MarkDispatched(ctx, "campfire1", "old-failed", "server1")
	store.mu.Lock()
	...
	store.MarkDispatched(ctx, "campfire1", "old-dispatched", "server1")
	store.mu.Lock()
	...
	store.MarkDispatched(ctx, "campfire1", "recent-fulfilled", "server1")
	store.mu.Lock()
...
func TestMemoryDispatchStore_ConcurrentMarkDispatched(t *testing.T) {
	...
	for i := 0; i < goroutines; i++ {
		go func() {
			defer wg.Done()
			inserted, _ := store.MarkDispatched(ctx, "campfire1", "msg1", "server1")
			if inserted {
				mu.Lock()
				insertedCount++
				mu.Unlock()
