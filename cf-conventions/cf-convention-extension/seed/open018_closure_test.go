...
func TestOPEN018Closure_SeedInL3(t *testing.T) {
	...
	const seedPkg = "github.com/campfire-net/campfire/cf-conventions/cf-convention-extension/seed"
	for _, dep := range strings.Split(strings.TrimSpace(string(out)), "\n") {
		if dep == seedPkg {
			t.Errorf("OPEN-018 REGRESSION: cf-convention (L2) imports cf-convention-extension/seed (L3).\n"+
