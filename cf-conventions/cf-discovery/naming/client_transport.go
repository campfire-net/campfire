...
type ResolverClientOptions struct {
	...
	// If empty, discovery.DefaultBeaconDir() is used.
	BeaconDir string
...
func autoJoinViaClient(client *protocol.Client, campfireID string, beaconDirOverride string, configTransportFunc func(string) protocol.Transport) error {
	...
	if beaconDir == "" {
		beaconDir = discovery.DefaultBeaconDir()
	}
	beacons, err := discovery.Scan(beaconDir)
	if err != nil {
...
func transportFromBeacon(b discovery.Beacon) (protocol.Transport, error) {
