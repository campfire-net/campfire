...
func TestIntroduceMeDeclaration(t *testing.T) {
	...
	if decl.SignerType != convention.SignerMemberKey {
		t.Errorf("signer_type: want %q, got %q", convention.SignerMemberKey, decl.SignerType)
	}
...
func TestIdentityDeclarations(t *testing.T) {
	...
	for i, want := range wantOps {
		...
		if decls[i].SignerType != convention.SignerMemberKey {
			t.Errorf("decls[%d].SignerType: want %q, got %q", i, convention.SignerMemberKey, decls[i].SignerType)
		}
	}
}
...
func argMap(args []convention.ArgDescriptor) map[string]convention.ArgDescriptor {
	m := make(map[string]convention.ArgDescriptor, len(args))
	for _, a := range args {
		m[a.Name] = a
	}
	return m
