...
var readCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if readFollow {
			return runFollowMode(entries, agentID, s, fieldSet, readAll, readPeek, tagFilter)
		}
		return runOneShotMode(campfireIDs, entries, agentID, s, fieldSet, readAll, readPeek, tagFilter)
	},
}
...
func init() {
	...
	readCmd.Flags().StringArray("tag", nil, "filter messages by tag (OR semantics, repeatable; use * suffix for prefix match)")
	readCmd.Flags().StringArray("exclude-tag", nil, "exclude messages with tag (repeatable; use * suffix for prefix match)")
