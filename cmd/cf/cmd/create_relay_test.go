...
func TestCreateRelay_OutputsBeaconAndRecordsMembership(t *testing.T) {
	...
	beaconStr, relayEndpoint, err := registerOnRelay(cf, agentID, agentStore, relay.URL, "test relay campfire")
	if err != nil {
...
func TestCreateRelay_LsShowsP2PHTTP(t *testing.T) {
	...
	_, _, err = registerOnRelay(cf, agentID, agentStore, relay.URL, "ls test campfire")
	if err != nil {
		t.Fatalf("registerOnRelay: %v", err)
	}
...
func TestCreateRelay_SecondAgentCanJoinAndRead(t *testing.T) {
	...
	_, _, err = registerOnRelay(cf, creatorID, creatorStore, relay.URL, "e2e test")
	if err != nil {
		t.Fatalf("registerOnRelay: %v", err)
	}
